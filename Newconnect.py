...
AUTOMATABLE_PARQUET = "automatable_use_cases.parquet"
NON_AUTOMATABLE_PARQUET = "non_automatable_use_cases.parquet"
...
def convert_excel_to_parquet(excel_file, parquet_file, chunk_size=CHUNK_SIZE):
    if Path(parquet_file).exists():
        logging.info(f"Parquet file {parquet_file} already exists, skipping conversion")
        return
    logging.info(f"Converting {excel_file} to {parquet_file}")
    # Read Excel in chunks and write to Parquet
    for i, chunk in enumerate(pd.read_excel(excel_file, sheet_name=0, chunksize=chunk_size)):
        if i == 0:
            chunk.to_parquet(parquet_file, index=False)
        else:
            chunk.to_parquet(parquet_file, mode='append', index=False)
        logging.info(f"Processed chunk {i + 1} for {excel_file}")
...
# Step 3: Process chunk of descriptions
...
def main():
    ...
    convert_excel_to_parquet(AUTOMATABLE_FILE, AUTOMATABLE_PARQUET)
    convert_excel_to_parquet(NON_AUTOMATABLE_FILE, NON_AUTOMATABLE_PARQUET)
    
    # Read Parquet files with dask
    logging.info("Reading Parquet files")
    automatable_df = dd.read_parquet(AUTOMATABLE_PARQUET)
    non_automatable_df = dd.read_parquet(NON_AUTOMATABLE_PARQUET)
