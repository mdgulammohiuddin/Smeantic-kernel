...
def store_embeddings_in_faiss_task(original_text: str, embedding_vector: list[float]):
    ...
    """
    # original_text, embedding_vector = text_and_embedding # This line is removed as parameters are now separate
    ...
    if not OPENAI_API_KEY:
        custom_logger.error("store_embeddings_in_faiss_task: OPENAI_API_KEY not found. Cannot initialize OpenAIEmbeddings for FAISS constructor.")
        raise ValueError("OPENAI_API_KEY is not configured for FAISS store setup.")

    try:
        # This embedding model is for the FAISS constructor, which requires an embedding_function.
        # Even when adding pre-computed embeddings, FAISS requires this.
        embeddings_model_for_faiss_constructor = OpenAIEmbeddings(model="text-embedding-ada-002", openai_api_key=OPENAI_API_KEY)
        
        embedding_dim = len(embedding_vector)
        if embedding_dim == 0:
            custom_logger.error("store_embeddings_in_faiss_task: Embedding vector dimension is 0. Cannot create FAISS index.")
            raise ValueError("Cannot initialize FAISS with a 0-dimension embedding.")
        ...
        vector_store = FAISS(
            embedding_function=embeddings_model_for_faiss_constructor, # Pass the model object directly
            index=index,
            docstore=docstore,
            index_to_docstore_id=index_to_docstore_id,
        )
        
        # Pass text_embeddings as a list of (text, embedding) tuples
        vector_store.add_embeddings(text_embeddings=[(original_text, embedding_vector)])

        custom_logger.info(f"store_embeddings_in_faiss_task: Successfully added text and embedding to FAISS. Index has {vector_store.index.ntotal} entries.")
